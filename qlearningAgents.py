...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        #print("Old Q for state ",state," and action ",legalActions[ai],": ",self.Q[state][ai])
        #print("self.Q[state][ai] ",self.Q[state][ai])
        #print("(self.Q[nextState]) ",(self.Q[nextState]))
        #print("alpha ",self.alpha)
        #print("thing we're adding ",self.alpha*(reward + self.discount*np.max(self.Q[nextState])-self.Q[state][ai]))
        if nextState=='TERMINAL_STATE':
            #print("The next state is the terminal state.")
            thingie=0
        else:
            thingie=np.max(self.Q[nextState])
        self.Q[state][ai]+=self.alpha*(reward + self.discount*thingie-self.Q[state][ai])
        #print("reward ",reward)
